...
class TCN(nn.Module):
    ...
    def get_loss(self, x, y): 
        ...
        x = self.get_features(x)  # out: (N_batch, Length, Class)
...
class LSTM(nn.Module):
    def __init__(self, input_size, config, n_layer=2, bidirectional=True, n_classes=5):
        super(LSTM, self).__init__()
        
        self.n_direct = int(bidirectional)+1
        self.hidden_dim = config['hyper_params']['hidden_dim']
        self.mask = torch.ones((config["data_loader"]["args"]["batch_size"], config['hyper_params']['seq_len'])).byte().cuda()
        self.crf = CRF(n_classes)

        self.lstm = nn.LSTM(input_size = input_size, hidden_size = self.hidden_dim, batch_first=True, num_layers = n_layer, bidirectional=bidirectional)
        self.fc = nn.Linear(self.hidden_dim*self.n_direct, n_classes)
