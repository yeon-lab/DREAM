import numpy as np
import torch
from base.base_trainer_proposed_method_record import BaseTrainer
...
class Trainer(BaseTrainer):
    ...
    def _train_feature_net(self, epoch):
        ...
        for batch_idx, (x, y, d) in enumerate(self.data_loader):
            ...
            all_loss, dict_losses = self.feature_net.get_losses(x, y, d)
            ...
            self.writer_featurenet.add_scalar("Loss/train", loss.item(), epoch)
            for key, value in dict_losses.items():
                self.writer_featurenet.add_scalar(key+"/train", value.item(), epoch)
    ...
    def _valid_feature_net(self):
        ...
        with torch.no_grad():
            ...
            for batch_idx, (x, y, _) in enumerate(self.valid_loader):
                ...
                self.writer_featurenet.add_scalar("Loss/valid", loss.item(), epoch)
...
##########################################
    ...
    def _train_classifier(self, epoch):
        ...
        for batch_idx, (x, y, _) in enumerate(self.data_loader):
            ...
            self.writer_classifier.add_scalar("Loss/train", loss.item(), epoch)
            self.train_metrics.update('loss', loss.item())
