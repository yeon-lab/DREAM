...
class p_decoder(nn.Module):
    def __init__(self, in_dim, out_dim):
        ...
        torch.nn.init.xavier_normal_(self.fc21[0].weight)
        self.fc21[0].bias.data.zero_()
        torch.nn.init.xavier_normal_(self.fc22[0].weight)
...
class DIVA(nn.Module):
    ...
    def get_losses(self, x, y, d):        
        ...
        for i in range(self.seq_len):
            ...
            dict_losses['CE_y'] += CE_y      
            dict_losses['CE_x'] += CE_x 
            dict_losses['KL_zd'] += zd_p_minus_zd_q 
            dict_losses['KL_zy'] += zy_p_minus_zy_q 
            dict_losses['Conts_loss'] += contrastive_loss(features, y_target)*self.const_weight

        
        all_losses = dict_losses['DIVA_loss']+dict_losses['Conts_loss']
        dict_losses['All_loss'] = all_losses

        return all_losses, dict_losses
