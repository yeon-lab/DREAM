...
class IITNet(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.classifier = PlainLSTM(config, hidden_dim=config['hyper_params']['hidden_dim'], num_classes=config['hyper_params']['num_classes'])
...
class PlainLSTM(nn.Module):
    def __init__(self, config, hidden_dim, num_classes):
        ...
        self.bidirectional = config['hyper_params']['bidirectional']

        self.input_dim = 128

        # architecture
        self.lstm = nn.LSTM(self.input_dim, self.hidden_dim, batch_first=True, num_layers=self.num_layers, bidirectional=config['hyper_params']['bidirectional'])
