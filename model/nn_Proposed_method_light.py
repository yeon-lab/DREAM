...
class TCN(nn.Module):
    def __init__(self, input_size, num_channels, config, d_type, n_classes=5):
        ...
        self.diva = DIVA(params['zy_dim'], config, d_type)
        
        
    def forward(self, x):
        """Inputs have to have dimension (batch_size, n_features, sequence_length)"""
        x = self.diva.get_features(x)
